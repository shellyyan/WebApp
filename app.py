#!/usr/bin/env python3  #使此python文件能直接在linux等上直接运行
# -*- coding:utf-8 -*-  #编码方式为utf-8
__author__='shellyyan'
'''
async web application
'''
#日志等级，默认为WARNING,只有指定级别或更高级的才会被追踪记录
import logging;
...
@asyncio.coroutine
def init(loop):
    app=web.Application(loop=loop)#创建一个循环类型为消息循环的web应用对象
    ...
    return srv
loop=asyncio.get_event_loop()#loop使一个消息循环对象
